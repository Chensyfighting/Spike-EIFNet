...
class ConvLSTM(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, kernel_size):
        ...
        self.zero_tensors = {}
        # 定义卷积层，用于计算四个门（输入门、遗忘门、输出门和细胞门）
        self.Gates = nn.Conv2d(input_size + hidden_size, 4 * hidden_size, kernel_size, padding=pad)

    def forward(self, input_, prev_state=None):
        """
        - input_ (torch.Tensor): 输入张量，形状为 [batch, channel, height, width]。
        - prev_state (tuple, optional): 上一个时间步的状态，默认为 None。
        返回:
        - hidden (torch.Tensor): 当前时间步的隐藏状态。
        - cell (torch.Tensor): 当前时间步的细胞状态。
        """
        # 获取批量大小和空间大小
        batch_size = input_.data.size()[0]
        spatial_size = input_.data.size()[2:]

        # 如果没有提供上一个状态，则生成空的上一个状态
        if prev_state is None:

            # 创建零张量（如果尚未创建）
            state_size = tuple([batch_size, self.hidden_size] + list(spatial_size))
            if state_size not in self.zero_tensors:
                # 分配一个形状为 `spatial_size` 的零张量（如果尚未分配）
        ...
        # 计算当前细胞状态和隐藏状态
        cell = (remember_gate.to(input_.device) * prev_cell.to(input_.device)) + (
                    in_gate.to(input_.device) * cell_gate.to(input_.device))
...
class ConvLSTMSequenceModel(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, kernel_size=3, stride=1, padding=0,
                 recurrent_block_type='convlstm', activation='relu', norm=None, BN_momentum=0.1):
