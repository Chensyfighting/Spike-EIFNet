...
class ConfusionMatrix(object):
    ...
    # def accuracy(self):  # 分割正确的像素除以总像素  按照DS的说法，该函数实现的是平均类别的准确率而非整体准确率
    #     accuracy = 0.0
    #     for i in range(self.nclass):
    #         accuracy += self.M[i, i] / np.sum(self.M[i, :])
    #
    #     return accuracy / self.nclass
    def accuracy(self): # todo  根据DS修改 得到的是整体准确率
        # 计算每个真实类别被正确预测的次数 (TP) 和未被正确预测的次数 (FN)
        total_TP = np.sum(np.diag(self.M))
        total_FN = np.sum(self.M) - total_TP  # 总真实样本数 - TP
