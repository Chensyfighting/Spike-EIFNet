...
def load_files_in_directory(directory, t_interval=50):
    ...
    # 判断 imgs_dir 是否包含 'dir1' 或 'dir5'
    if 'dir1' in imgs_dir or 'dir5' in imgs_dir:
        # 如果包含 'dir1' 或 'dir5'，执行此段代码
        img_files = sorted(glob.glob(os.path.join(imgs_dir, "img_*.png")))
    else:
        # 否则，执行此段代码
    ...
    with ThreadPoolExecutor(max_workers=64) as executor:
        rgb_images = list(executor.map(lambda f: cv2.cvtColor(cv2.imread(f), cv2.COLOR_BGR2RGB), img_files))

    #  note 👆
    # return img_timestamp_event_idx, t_events, xyp_events, segmentation_mask_files
    return img_timestamp_event_idx, t_events, xyp_events, segmentation_mask_files, rgb_images   # note ===================


def load_events(t_file, xyp_file):
    """加载事件数据"""
    # 文件 events.dat.t 保存了每个事件的时间戳（单位为纳秒，数据类型为 int64）。
    # 文件 events.dat.xyp 保存了每个事件的 x 坐标、y 坐标和极性，数据类型为 uint8 以节省存储空间。极性是 0 或 1。
    # 我们需要计算事件的总数，因为 memmap 文件不会直接告诉我们这一信息。
    # 计算方式是获取时间戳文件的大小，并除以 8（因为每个时间戳占用 8 字节）。
...
def extract_events_from_memmap(t_events, xyp_events, img_idx, img_timestamp_event_idx, fixed_duration=False,
    ...
    """
    从内存映射文件中提取事件。
    该函数根据给定的图像索引和时间戳事件索引，从内存映射文件中提取出发生在两次图像捕获之间的事件。
    如果设置了fixed_duration参数，函数将根据固定的时间间隔来提取事件；否则，根据事件的数量来提取。
    返回:
    - events_between_imgs: 发生在两次图像捕获之间的事件。
    """
    # timestep, event_idx = img_timestamp_event_idx[img_idx]
    # _, event_idx_before = img_timestamp_event_idx[img_idx - 1]
    # 根据是否使用固定时间间隔，确定事件的起始和结束索引
    if fixed_duration:
        timestep, event_idx, event_idx_before = img_timestamp_event_idx[img_idx]
        event_idx_before = max([event_idx_before, 0])
    else:
        timestep, event_idx, _ = img_timestamp_event_idx[img_idx]   # 加载当前img_idx索引对应的事件数据
        event_idx_before = max([event_idx - nr_events, 0])
    # 从t_events和xyp_events中提取事件，并将它们连接起来
