...
def normalize_rgb(rgbs):
    """Normalize RGB tensor similar to normalize_voxel_grid"""
    rgbs = rgbs / 255.0

    # 对每个通道计算均值和标准差
    means = rgbs.mean(dim=(1, 2), keepdim=True)
    stddevs = rgbs.std(dim=(1, 2), keepdim=True)

    # 对每个通道进行标准化
    rgbs = (rgbs - means) / stddevs

    return rgbs

def generate_voxel_grid(events, shape, nr_temporal_bins, separate_pol=True):
    """
    根据事件集生成时间轴的双线性插值的体素网格。在时间上进行插值，最终的体素网格是一个3D数据结构，大小为 (nr_temporal_bins, height, width)
    :param events: 一个[Nx4]的NumPy 数组，每行包含一个事件，格式为[x,y,t,p]
    :param nr_temporal_bins:时间维度，将时间轴离散为固定数量的时间段
    :param shape:体素网格的尺寸
    :param separate_pol:如果为True，返回正极性和负极性分开的体素网格
    :return: 生成的体素网格
    """
    height, width = shape
    ...
    assert (height > 0)
    # 初始化正极性和负极性的体素网格
    voxel_grid_positive = np.zeros((nr_temporal_bins, height, width), np.float32).ravel()  # .ravel()展平为1D数组方便索引操作
    voxel_grid_negative = np.zeros((nr_temporal_bins, height, width), np.float32).ravel()

    # 归一化事件时间戳，将原始时间戳映射到[0,nr_temporal_bins-1]的范围
    ...
    # ts = events[:, 2]
    # print(ts[:10])
    ts = (nr_temporal_bins - 1) * (
                events[:, 2] - first_stamp) / deltaT  # ts是归一化的事件时间戳，将原始时间戳映射到[0,nr_temporal_bins-1]的范围
    # 处理极性值，确保极性为 +1 或 -1
    pols = events[:, 3]
    pols[pols == 0] = -1  # 如果极性为 0，将其替换为 -1

    tis = ts.astype(np.int64)  # 时间戳的整数部分
    dts = ts - tis  # 时间戳的小数部分，用于时间域的双线性插值
    vals_left = np.abs(pols) * (1.0 - dts)  # note 对应当前整数时间戳的体素值，权重为 1.0 - dts
    vals_right = np.abs(pols) * dts  # note 对应下一个时间戳的体素值，权重为 dts
    pos_events_indices = pols == 1

    # Positive Voxels Grid
    valid_indices_pos = np.logical_and(tis < nr_temporal_bins, pos_events_indices)  # 筛选正极性事件且其时间戳小于 nr_temporal_bins
    valid_pos = (xs < width) & (xs >= 0) & (ys < height) & (ys >= 0) & (ts >= 0) & (
                ts < nr_temporal_bins)  # 筛选事件的空间坐标（x, y）和时间戳（t）都在有效范围内的事件
    valid_indices_pos = np.logical_and(valid_indices_pos, valid_pos)  # 最终的 valid_indices_pos 是满足所有条件的事件

    np.add.at(voxel_grid_positive, xs[valid_indices_pos] + ys[valid_indices_pos] * width +
              tis[valid_indices_pos] * width * height, vals_left[valid_indices_pos])  # 使用np.add.at将事件值加到对应的体素中
    # 将时间戳+1（计算下一个时间戳）后再次计算，目的是确保事件在两个连续的时间窗口中都被正确地累加到体素网格中。这通常用于处理事件相机数据时的时间冗余或平滑过渡，确保每个事件在相邻的时间窗口中都能被考虑到
    ...
    # 重塑体素网格形状
    voxel_grid_positive = np.reshape(voxel_grid_positive, (nr_temporal_bins, height, width))  # 将1D的体素网格重塑为3D的形状
    ...
    return voxel_grid  # 形状（5，260，346），即(nr_temporal_bins, height, width)
